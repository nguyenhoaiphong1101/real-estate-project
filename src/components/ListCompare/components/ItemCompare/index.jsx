...
function ItemCompare(props) {
    ...
    const changeSearch = (e) => {
        setListShow(e.target.value === "" ? listDepartment : listDepartment.filter(item => item.title?.toLowerCase().includes(e.target.value.toLowerCase())))
    }
        ...
        <>
            ...
            >
                <div className="modal-search">
                    <Input.Search placeholder="Nhập tên sản phẩm" onChange={changeSearch} style={{ width: "100%" }} />
                    <List className='list'
                        dataSource={listShow}
                        renderItem={item => (
                            <ItemModal item={item} />
...
function ItemModal({ item }) {
    ...
    const addCompare = () => {
        ...
        if (listCompare.filter(it => it === item.id).length === 0 && listCompare.length < 3) {
            temp.push(item.id)
