    ...
    if (listCompare?.length !== listItem?.length) {
      if (listCompare.length === 0) {
        ...
        getListCompare
          ...
          .then((res) => {
            setListItem(res);
            dispatch(changeCompareDetail(res));
            setCollapse(false);
          });
      }
    }
  ...
  const clickCompare = () => {
    if (listItem.length <= 1) {
      ...
      if (listItem.length === 2) {
        ...
        history.push(
          `/so-sanh/${listItem[0].id},${listItem[1].id},${listItem[2].id}`
        );
      }
    }
  };
    ...
    <div>
      {collapse ? (
        <div className="list-mini">
          <p className="title" onClick={isCollapse}>
            So sánh ({listCompare.length}){" "}
        ...
        <div className="list-compare">
          ...
          <Row className="cart">
            ...
            <Col className="button-compare" span={6}>
              <div>
                <Button className="button-compare" onClick={clickCompare}>
                  So sánh ngay
                </Button>
                <span className="clear" onClick={deleteAll}>
                  Xóa các lựa chọn
                </span>
              </div>
            </Col>
