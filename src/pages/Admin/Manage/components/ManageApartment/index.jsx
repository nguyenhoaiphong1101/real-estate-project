...
function ManageApartment(props) {
      ...
      render: (text, record) =>
        record?.is_highlight ? (
          <i
            ...
            onClick={() => changeHighlight(record)}
      ...
      render: (text, record) => (
        <Space size="middle">
          ...
          {text.status === "PENDING" ? (
            <Button
              className="admin-btn-approve"
              onClick={() => showModalStatus(record)}
            >
              Duyệt
            </Button>
          ) : null}
  ...
  const listApartment = useSelector(
    (state) => state.admin.apartment.listApartment
  );
  ...
  return (
    <div className="admin-manage-apartment">
      <Modal
        className="modal-apartment"
        width={800}
        title={typeButton === "ADD" ? "Thêm căn hộ" : "Chỉnh sửa căn hộ"}
        visible={isModalVisible}
        onOk={typeButton === "ADD" ? addApartment : changeApartment}
            ...
            <Row>
              ...
              <Col offset={2} span={11}>
                ...
                <Form.Item name="type_apartment">
                  <Select
                    className="form-control select"
                    onChange={setValueTypeApartment}
                  >
          ...
          <Row>
            ...
            <Col span={11}>
              ...
              <Form.Item name="province">
                <Select
                  className="form-control select"
                  onChange={setValueProvinceFilter}
                >
                  {listProvince.map((item) => (
                    <Option key={item?.id} value={item.id}>
                      {item?.name}
            ...
            <Col offset={2} span={11}>
              ...
              <Form.Item name="district">
                <Select
                  className="form-control select"
                  onChange={setValueDistrictFilter}
                >
                  {listDistrict.map((item) => (
                    <Option key={item?.id} value={item.id}>
                      {item?.name}
            ...
            <Col span={11}>
              ...
              <Form.Item name="category">
                <Select
                  className="form-control select"
                  onChange={setValueCategoryFilter}
                >
                  {listCategory.map((item) => (
                    <Option key={item?.id} value={item.id}>
                      {item?.name}
            ...
            <Col offset={2} span={11}>
              ...
              <Form.Item name="type_apartment">
                <Select
                  className="form-control select"
                  onChange={setValueTypeApartmentFilter}
                >
                  <Option value="BUY">Bán</Option>
                  <Option value="RENT">Cho thuê</Option>
                </Select>
      ...
      >
        <p>
          Bạn có chắc chắn muốn xóa ? (Sau khi xóa bài viết sẽ có trạng thái
          CLOSE)
        </p>
      </Modal>
      <Modal
        className="modal-apartment"
        title="Thông báo"
        visible={isVisibleNotiReset}
        getContainer={false}
        forceRender
        onOk={handleOkNotiReset}
        onCancel={handleCancelNotiReset}
        okText="Xác nhận"
      >
        <p>
          Bạn có chắc chắn muốn mở lại bài viết ? (Sau khi mở lại bài viết sẽ có
          trạng thái OPEN)
        </p>
      </Modal>
      <Row>
          ...
          <Row>
            <Col span={2}>
              <Button
                ...
                className="admin-btn-add-apartment"
              >
                Lọc <AlignLeftOutlined />
              </Button>
            </Col>
            <Col offset={1} span={3}>
              <Button
            ...
            <Col offset={1} span={4}>
              ...
              >
                Thêm căn hộ <PlusOutlined />
              </Button>
            </Col>
            <Col offset={5} span={2}>
              <p style={{ margin: "20px 0px 0px 20px" }}>Sắp xếp</p>
            </Col>
            ...
            <Col span={3}>
              <Select
                className="form-control select"
                onChange={(value) => onDirection(value)}
                defaultValue="ASC"
              >
