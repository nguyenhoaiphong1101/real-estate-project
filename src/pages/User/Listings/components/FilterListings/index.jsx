...
function FilterListings(props) {
  ...
  useEffect(() => {
    if (location.state?.from !== "/") {
      ...
      searchFilter(
        filter.valueCategory,
        filter.valueProvince,
        filter.valueDistrict,
        filter.valuePrice,
        filter.valueArea,
        filter.inputSearch
      );
  ...
  const searchFilter = async (
    ...
    await dispatch(
      loadListSearch({
        type_apartment: props.typeListing === "Nhà đất bán" ? "BUY" : "RENT",
        area_from:
          valueArea?.from && valueArea?.id !== "-1"
            ? valueArea?.from
            : undefined,
        area_to:
          valueArea?.to && valueArea?.id !== "-1" && valueArea?.to !== -1
            ? valueArea.to
            : undefined,
        category_id:
          valueCategory?.id && valueCategory?.id !== "-1"
            ? valueCategory?.id
            : undefined,
        district_id:
          valueDistrict?.id && valueDistrict?.id !== "-1"
            ? valueDistrict?.id
            : undefined,
        price_from:
          valuePrice?.from && valuePrice?.id !== "-1"
            ? valuePrice?.from
            : undefined,
        price_to:
          valuePrice?.to && valuePrice?.id !== "-1" && valuePrice?.to !== -1
            ? valuePrice.to
            : undefined,
        province_id:
          valueProvince?.id && valueProvince?.id !== "-1"
            ? valueProvince?.id
            : undefined,
      ...
      <Collapse defaultActiveKey={["1"]} className="collapse collapse-filter">
        <Panel header={<h5 className="title">Lọc theo danh sách</h5>} key="1">
            ...
            <form className="filter-listings-form">
              <div className="form-group acr-custom-select">
                <Input
                  style={{
                    ...
                    borderRadius: "8px",
                    border: "0",
                    boxShadow: "0 2px 9px 0 rgba(34,12,43,0.1)",
                  }}
                  className="input"
              ...
              <div className="form-group acr-custom-select">
                <SelectCustom
                  title="Mức giá"
                  value={valuePrice}
                  onHandleChange={changeValuePrice}
                  options={price}
                />
              </div>
              <div className="form-group acr-custom-select">
                <SelectCustom
                  title="Diện tích"
                  value={valueArea}
                  onHandleChange={changeValueArea}
                  options={acreage}
                />
              </div>
            </form>
            <form
              className={`filter-listings-form ${filterHigh ? "show" : "none"}`}
            >
              {/* <div className="form-group acr-custom-select">
          ...
          <div className="filter-high">
            ...
            >
              {filterHigh ? "Thu gọn tìm kiếm" : "Tìm kiếm nâng cao"}{" "}
              <i
                className={`fas ${filterHigh ? "fa-sort-up" : "fa-sort-down"}`}
              ></i>
            </span>
          </div>
          <div className="filter">
            ...
            />
            <i onClick={clearSearch} className="fas fa-sync-alt icon"></i>
          </div>
        </Panel>
      </Collapse>
    </div>
